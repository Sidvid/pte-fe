...
const StudentTaskView: React.FC = () => {
                ...
                questions: [
                  {
                    ...
                    index: 0,
                    type: "FIB_RW",
                    data: { text: "The tomato is probably the most widely grown }} by the home gardener because of its {{ value, many uses, and relative ease of }}. Originating in Central and South America, the tomato was domesticated in Mexico. There are many related wild species in South America. \"Tomati\" was the name {{ by Native Americans." },
                    extra: {},
  ...
  if (!taskDetails || !taskDetails.questions || taskDetails.questions.length === 0) {
    return (
      <div className="p-6">
        <Alert 
        ...
        <div className="bg-white p-6 rounded-xl shadow-md">
          <h2 className="text-xl font-semibold mb-4">Task: {taskDetails?.title || "Sample Task"}</h2>
    ...
    <div className="p-6">
      ...
      <div className="bg-white p-6 rounded-xl shadow-md">
        ...
        {taskDetails?.questions[currentStep] && (
          <div className="mb-6">
            <div className="mt-2 text-sm text-gray-500">Question #{taskDetails.questions[currentStep].sNo || currentStep + 1}</div>
            {taskDetails.questions[currentStep].type === "FIB_RW" && (
              <FillInTheBlanks text={typeof taskDetails.questions[currentStep].data === 'object' 
                ? (taskDetails.questions[currentStep].data as any).text 
                : taskDetails.questions[currentStep].data} 
              />
            )}
            {taskDetails.questions[currentStep].type === "FIB_DRAG_AND_DROP" && (
              <FibDragDrop questions={[taskDetails.questions[currentStep]]} />
            )}
            {/* Handle other question types if needed */}
            {![ "FIB_RW", "FIB_DRAG_AND_DROP"].includes(taskDetails.questions[currentStep].type) && (
              <div>
                <p>Unsupported question type: {taskDetails.questions[currentStep].type}</p>
                <pre>{JSON.stringify(taskDetails.questions[currentStep], null, 2)}</pre>
              </div>
            )}
          </div>
        )}
