...
function ViewSingleCollectionQuestions() {
  ...
  const questionTypeMappings = [
    {
      includes: "Describe Image",
      value: QuestionMapping.DESCRIBE_IMAGE,
      key: "DESCRIBE_IMAGE" as const,
    },
    {
      includes: "Read Aloud",
    ...
    {
      includes: "No Flow Read Aloud",
      value: QuestionMapping.NO_FLOW_READ_ALOUD,
      key: "NO_FLOW_READ_ALOUD" as const,
    },
    {
      includes: "Repeat Sentence",
      value: QuestionMapping.REPEAT_SENTENCE,
      key: "REPEAT_SENTENCE" as const,
    },
    {
      includes: "Retell Lecture",
      value: QuestionMapping.RETELL_LECTURE,
      key: "RETELL_LECTURE" as const,
    },
    {
      includes: "Summarize Spoken",
      value: QuestionMapping.SUMMARIZE_SPOKEN,
      key: "SUMMARIZE_SPOKEN" as const,
    },
    {
      includes: "FIB - Drag & Drop",
      value: QuestionMapping.FIB_DRAG_AND_DROP,
      key: "FIB_DRAG_AND_DROP" as const,
    },
    {
      includes: "Listening Blanks",
      value: QuestionMapping.FIB_LISTENING,
      key: "FIB_LISTENING" as const,
    },
    {
      includes: "Highlight Incorrect Words",
      value: QuestionMapping.HIGHLIGHT_SUMMARY,
      key: "HIGHLIGHT_SUMMARY" as const,
    },
    {
      includes: "Write From Dictation",
      value: QuestionMapping.WRITE_FROM_DICTATION,
      key: "WRITE_FROM_DICTATION" as const,
    },
    {
      includes: "Dictation Prediction",
      value: QuestionMapping.DICTATION_PREDICTION,
      key: "DICTATION_PREDICTION" as const,
    },
    {
      includes: "WFD Prediction",
      value: QuestionMapping.WFD_PREDICTION,
      key: "WFD_PREDICTION" as const,
    },
    {
      includes: "FIB - Drop Down",
  ...
  const getQuestionTypeInfo = () => {
    if (!state?.title) {
      return {
        value: QuestionMapping.DESCRIBE_IMAGE,
        key: "DESCRIBE_IMAGE" as QuestionMappingType,
      };
    }
  ...
  const renderQuestion = useMemo(() => {
    switch (questionType) {
      case "READ_ALOUD":
        return (
          <ReadAloud questions={questions!} onDelete={handleDeleteQuestion} />
        );
      ...
      case "FIB_DRAG_AND_DROP":
        return (
          <FibDragDrop questions={questions!} onDelete={handleDeleteQuestion} />
        );
      case "FIB_LISTENING":
        return (
          <FibListening
