...
function SummarizeSpokenQuestion() {
  ...
  useEffect(() => {
    if (audioFile || scriptText.trim()) {
      setCreateQuestionData({ 
        text: scriptText || "Listen to the audio and summarize the main points", 
        type: audioFile || "",
        extra: scriptText
      });
    } else {
  ...
  }, [questionData.type, questionData.text]);
    ...
    <div className="flex flex-col gap-6 p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
      <div className="mb-2">
        ...
        <p className="text-sm text-gray-600">
          Upload an audio file for students to listen to and summarize.
        </p>
      </div>
      ...
      <div className="mt-6">
        <label className="block text-base font-bold text-gray-800 mb-2">
          Audio Script/Transcript (Optional)
        </label>
        <p className="text-sm text-gray-600 mb-3">
          This is optional but helps with scoring and reference. Enter the full transcript of the audio.
        </p>
        <TextArea
          value={scriptText}
          onChange={(e) => setScriptText(e.target.value)}
          placeholder="Enter the script or transcript of the audio for reference..."
          rows={8}
          showCount
          maxLength={2000}
      ...
      <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <p className="text-base font-semibold text-gray-700 mb-2">About Summarize Spoken Text:</p>
        <p className="text-gray-600">
          Students will listen to the audio (60-90 seconds) and write a summary of 50-70 words. 
          They need to capture the main points and supporting details.
        </p>
      </div>
    </div>
  );
...
export default SummarizeSpokenQuestion;
