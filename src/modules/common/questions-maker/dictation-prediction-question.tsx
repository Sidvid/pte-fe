...
function DictationPredictionQuestion() {
  ...
  useEffect(() => {
    if (audioFile && script) {
      setCreateQuestionData({ 
        text: JSON.stringify({ script, audio: audioFile.name }), 
        type: audioFile,
        extra: JSON.stringify({ transcript: script })
      });
    } else if (script) {
      setCreateQuestionData({ 
        text: script, 
    ...
    <div className="flex flex-col gap-6 p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
      <div className="mb-2">
        ...
        <p className="text-sm text-gray-600">
          Upload an audio file for students to listen to and predict what they hear.
        </p>
      </div>
      ...
      <div className="mt-6">
        ...
        <p className="text-sm text-gray-600 mb-3">
          Enter the text that will be used for dictation prediction exercise.
        </p>
        
        <TextArea
          value={script}
          onChange={(e) => setScript(e.target.value)}
          placeholder="Enter the dictation text that students will hear and need to predict/type..."
          rows={6}
          showCount
          maxLength={1000}
      ...
      <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <p className="text-base font-semibold text-gray-700 mb-2">About Dictation Prediction:</p>
        <p className="text-gray-600">
          Students will listen to the audio and predict/type what they hear. 
          This helps improve listening and writing skills.
        </p>
      </div>
    </div>
  );
...
export default DictationPredictionQuestion;